...
class String
  ...
  def caesar(shift)
    ...
    alphabet = letters * 2 + letters.upcase * 2    #works for uppercase
    cipher = chars.map do |ch|
      ch_idx = alphabet.index(ch)
      alphabet[ch_idx + shift]
    end
    cipher.join
  end
...
class Hash
  ...
  def difference(other_hash)
    diff = {}
    each { |k,v| diff[k] = v unless other_hash.key?(k) }
    other_hash.each { |k,v| diff[k] = v unless self.key?(k) }
    diff
  end
